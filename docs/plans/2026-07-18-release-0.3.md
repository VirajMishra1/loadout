...
4. Stable, Power, and Maximum are re-evaluated against the trusted catalog whenever
   ...
   are recorded.
5. `loadout mcp-recipe --no-key` lists only reviewed recipes requiring no provider or
   service credential. The release includes Playwright MCP and Chrome DevTools MCP.
