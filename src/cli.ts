...
program.command("sync")
  ...
  .action(async (options: { manifest: string; lock: string; yes?: boolean; approveRisk?: boolean }) => {
    ...
    console.log(JSON.stringify({ manifest: plan.manifest, packages: plan.packages.map((entry) => ({ ...entry.plan, safety: entry.safety })), mcpChanges: plan.mcpPlans.map((entry) => ({ packageId: entry.packageId, path: entry.plan.path, changes: entry.plan.changes, warnings: entry.plan.warnings })), skipped: plan.skipped, policyViolations: plan.policyViolations }, null, 2));
...
program.command("codex-mcp-config")
  ...
  .action(async (options: { config: string; name: string; command?: string; url?: string; arg: string[]; env: string[]; yes?: boolean }) => {
    ...
    if (!options.yes) return console.log("Dry run only. Re-run with --yes to add this Codex MCP server.");
