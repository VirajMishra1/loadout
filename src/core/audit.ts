...
export async function readLockfile(path = "loadout.lock"): Promise<LoadoutLockfile> {
  try { return parseLockfile(JSON.parse(await readFile(resolve(path), "utf8"))); }
  catch (error) { throw new Error(`Invalid Loadout lockfile at ${resolve(path)}: ${error instanceof Error ? error.message : String(error)}`); }
}

function expectedPackages(manifest: LoadoutManifest): string[] {
  return manifest.packages.filter((pkg) => pkg.enabled !== false).map((pkg) => pkg.id).sort();
}

export async function auditLoadout(manifestPath = "loadout.json", lockPath = "loadout.lock"): Promise<AuditReport> {
  const [manifest, lockfile, state] = await Promise.all([readManifest(manifestPath), readLockfile(lockPath), readInstallState()]);
  const findings: AuditFinding[] = [];
  if (lockfile.manifestName !== manifest.name) findings.push({ level: "error", code: "manifest-name", message: `Lockfile belongs to '${lockfile.manifestName}', not '${manifest.name}'.` });
  const expected = expectedPackages(manifest); const locked = lockfile.packages.map((pkg) => pkg.id).sort();
  for (const id of expected.filter((id) => !locked.includes(id))) findings.push({ level: "error", code: "missing-lock", message: `Package '${id}' is enabled but missing from the lockfile.` });
  for (const id of locked.filter((id) => !expected.includes(id))) findings.push({ level: "error", code: "extra-lock", message: `Lockfile contains package '${id}' that is not enabled in the manifest.` });
  ...
  for (const lockedPackage of lockfile.packages) {
    ...
    if (manifestPackage && JSON.stringify(manifestPackage.source) !== JSON.stringify(lockedPackage.source)) findings.push({ level: "error", code: "source-mismatch", message: `Package '${lockedPackage.id}' source differs between manifest and lockfile.` });
    const installed = stateById.get(lockedPackage.id);
    if (!installed) { findings.push({ level: "warning", code: "not-installed", message: `Package '${lockedPackage.id}' is locked but not installed on this machine.` }); continue; }
    if ((lockedPackage.resolvedCommit ?? "") !== (installed.resolvedCommit ?? "")) findings.push({ level: "error", code: "commit-mismatch", message: `Package '${lockedPackage.id}' installed revision differs from the lockfile.` });
    ...
    for (const path of lockedFiles.keys()) if (!installedFiles.has(path)) findings.push({ level: "error", code: "missing-state-file", message: `Lockfile-managed path is missing from installed state: ${path}` });
    for (const file of installed.files) {
      if (lockedFiles.get(file.path) !== file.sha256) findings.push({ level: "error", code: "state-lock-mismatch", message: `Managed file record differs from lockfile: ${file.path}` });
      try {
        const digest = createHash("sha256").update(await readFile(file.path)).digest("hex");
        if (digest !== file.sha256) findings.push({ level: "error", code: "file-drift", message: `Managed file changed or is corrupt: ${file.path}` });
...
export function formatAuditReport(report: AuditReport): string {
  return [`Loadout audit: ${report.valid ? "PASS" : "FAIL"}`, ...report.findings.map((finding) => `${finding.level === "ok" ? "✓" : finding.level === "warning" ? "!" : "✗"} ${finding.message}`)].join("\n");
}
