 ...
 */
export async function planCodexMcpConfig(path: string, server: McpServer, name = server.name): Promise<CodexMcpConfigPlan> {
  safeName(name);
  if ((server.command ? 1 : 0) + (server.url ? 1 : 0) !== 1) throw new Error("Codex MCP server requires exactly one command or URL");
...
export async function applyCodexMcpConfigPlan(plan: CodexMcpConfigPlan): Promise<McpConfigSnapshot> {
  let existed = true; let content: string | undefined;
  try { content = await readFile(plan.path, "utf8"); }
  ...
  try { await writeFile(temporary, plan.proposed, { mode: 0o600 }); await rename(temporary, plan.path); }
