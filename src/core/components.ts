...
export async function discoverResources(root: string): Promise<ResourceSummary[]> {
  const base = resolve(root);
  const resources: ResourceSummary[] = [];
  async function visit(directory: string, depth: number, insideSkill: boolean): Promise<void> {
    if (depth > 5) return;
    const entries = await readdir(directory, { withFileTypes: true });
    const skill = insideSkill || entries.some((entry) => entry.name === "SKILL.md" && entry.isFile());
    for (const entry of entries) {
      if (entry.name === ".git" || entry.name === "node_modules") continue;
      const absolute = join(directory, entry.name);
      if (entry.isSymbolicLink()) {
        if (RESOURCE_DIRECTORIES.has(entry.name)) throw new Error(`Refusing symlinked resource directory: ${absolute}`);
        continue;
      }
      ...
      if (type) {
        for (const child of await readdir(absolute, { withFileTypes: true })) {
          if (child.isSymbolicLink()) throw new Error(`Refusing symlinked package resource: ${join(absolute, child.name)}`);
          if (!child.isFile() && !child.isDirectory()) continue;
          resources.push({ type, name: child.name.replace(/\.(md|json|ya?ml)$/i, ""), path: relative(base, join(absolute, child.name)).split(sep).join("/") });
        }
        continue;
      }
...
function safeSource(root: string, path: string): string {
  ...
  if (source !== base && !source.startsWith(`${base}${sep}`)) throw new Error(`Resource escapes package root: ${path}`);
  return source;
}

export async function buildUniversalPackagePlan(root: string, packageId: string, agents: DetectedAgent[]): Promise<InstallPlan> {
  const files: PlannedFile[] = [];
  const warnings: string[] = [];
  try {
    const skills = await buildSkillPlan(root, packageId, agents);
    files.push(...skills.files.map((file) => ({ ...file, componentType: "skill" as const, compatibility: "native" as const })));
    warnings.push(...skills.warnings);
  } catch (error) {
    if (!(error instanceof Error) || !error.message.startsWith("No SKILL.md found")) throw error;
  }
