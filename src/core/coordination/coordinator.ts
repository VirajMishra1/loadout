...
export async function emit(
  ...
  // Validate typed payload if present
  const payloadResult = validatePayload(options.type, options.payload);
  if (!payloadResult.success) {
    throw new Error(
      `Invalid payload for '${options.type}': ${payloadResult.error}`,
    );
  }
  ...
  await writeFile(coordLogPath(projectRoot), line, { flag: "a" });
...
export async function checkOwnershipConflicts(
  ...
  for (const path of paths) {
    ...
    // Any exclusive involvement = conflict
    conflicts.push({
      path,
