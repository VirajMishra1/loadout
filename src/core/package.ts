...
export async function inspectPackage(root: string): Promise<PackageInspection> {
  ...
  return { root: resolvedRoot, skills, mcpServers, counts: { skills: skills.length, mcpServers: mcpServers.length, manifests: manifests.length }, warnings };
}

export function formatPackageInspection(result: PackageInspection): string {
  const lines = [`Package: ${result.root}`, `Skills: ${result.counts.skills}`, `MCP servers: ${result.counts.mcpServers}`, `Manifests: ${result.counts.manifests}`];
