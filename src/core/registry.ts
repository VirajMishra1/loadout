...
export function parsePackageDescriptor(value: unknown): PackageDescriptor {
  ...
  if (typeof item.name !== "string" || !NAME.test(item.name))
    throw new Error(
      "Package name must use lowercase letters, numbers, dots, underscores, or dashes",
    );
  ...
  for (const field of ["dependencies", "devDependencies"] as const) {
    ...
    if (
      ...
      throw new Error(
        `Package ${field} must map names to exact semantic versions`,
      );
  }
  return {
    schemaVersion: 1,
    ...
    ...(item.dependencies
      ? { dependencies: item.dependencies as Record<string, string> }
      : {}),
    ...(item.devDependencies
      ? { devDependencies: item.devDependencies as Record<string, string> }
      : {}),
  };
}
...
export async function packPackage(root: string): Promise<PackedPackage> {
  ...
  const descriptor = parsePackageDescriptor(
    JSON.parse(await readFile(join(directory, "loadout-package.json"), "utf8")),
  );
...
export async function publishLocalPackage(
  ...
  const safety: UpdateSafetyAnalysis = await analyzeUpdateSafety(
    undefined,
    packed.root,
  );
  if (safety.approvalRequired && !options.approveRisk)
    throw new Error(
      `Publishing requires explicit risk approval: ${safety.findings
  ...
  const target = join(
    registryRoot(),
    packed.descriptor.name,
    packed.descriptor.version,
  );
  try {
    const existing = JSON.parse(
      await readFile(join(target, ".loadout-record.json"), "utf8"),
    ) as { digest?: string };
    if (existing.digest === packed.digest) return packed;
    throw new Error(
      `Registry version ${packed.descriptor.name}@${packed.descriptor.version} already exists with different content`,
    );
  } catch (error) {
    if (!(
      error &&
  ...
  try {
    ...
    await writeFile(
      join(temporary, ".loadout-record.json"),
      `${JSON.stringify({ name: packed.descriptor.name, version: packed.descriptor.version, digest: packed.digest, publishedAt: new Date().toISOString(), safety }, null, 2)}\n`,
    );
...
export async function resolveRegistryPackage(
  ...
  try {
    const record = JSON.parse(
      await readFile(join(path, ".loadout-record.json"), "utf8"),
    ) as { digest?: string };
    if (!record.digest) throw new Error("missing digest");
    const packed = await packPackage(path);
    if (packed.digest !== record.digest)
      throw new Error(
        "registry package content does not match its immutable digest",
      );
    return { path, digest: record.digest };
  } catch (error) {
    throw new Error(
      `Could not resolve registry package ${name}@${version}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

export async function searchLocalRegistry(
  query = "",
    ...
    for (const version of versions.sort()) {
      try {
        const descriptor = parsePackageDescriptor(
          JSON.parse(
            await readFile(
              join(registryRoot(), name, version, "loadout-package.json"),
              "utf8",
            ),
          ),
        );
...
export async function createRegistryBundle(
  root: string,
  ...
  const files = await Promise.all(
    packed.files.map(async (file) => ({
      ...file,
      content: (await readFile(resolve(packed.root, file.path))).toString(
        "base64",
      ),
    })),
  );
  ...
  try {
    for (const file of bundle.files) {
      ...
      if (target === root || !target.startsWith(`${root}${sep}`))
        throw new Error(
          `Registry bundle path escapes destination: ${file.path}`,
        );
      ...
      if (
        ...
        throw new Error(
          `Registry bundle file verification failed: ${file.path}`,
        );
...
function registryUrl(input: string): URL {
  ...
  if (url.protocol !== "https:" && !(loopback && url.protocol === "http:"))
    throw new Error(
      "Remote registries require HTTPS outside loopback development",
    );
...
export async function publishRemotePackage(
  root: string,
  registry: string,
  token: string,
  options: { approveRisk?: boolean } = {},
): Promise<{ name: string; version: string; digest: string }> {
  ...
  if (!response.ok)
    throw new Error(
      result.error ?? `Remote registry returned ${response.status}`,
    );
  return {
    name: result.name!,
