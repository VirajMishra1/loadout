...
export function detectSecretKinds(content: string): string[] {
  return SECRET_PATTERNS.filter((check) => check.pattern.test(content)).map(
    (check) => check.name,
  );
}
...
export async function analyzeInstallPlanSafety(
  plan: InstallPlan,
): Promise<UpdateSafetyAnalysis> {
  const roots = [...new Set(plan.files.map((file) => file.source))];
  const analyses = await Promise.all(
    roots.map((root) => analyzeUpdateSafety(undefined, root)),
  );
...
export async function analyzeUpdateSafety(
  ...
  if (scriptPaths.length)
    findings.push({
      ...
      category: "script",
      message:
        "Update adds or changes scripts that may execute during setup or use.",
      paths: scriptPaths,
    });
  if (hookPaths.length)
    findings.push({
  ...
  if (domains.size)
    findings.push({
      ...
      category: "domain",
      message:
        "Update references network domains; verify and explicitly approve the new network surface.",
      paths: changed.filter(
        (path) =>
          collectDomains(newFiles.get(path)?.toString("utf8") ?? "").length > 0,
      ),
      names: [...domains].sort(),
    });
  if (envNames.size)
    findings.push({
      ...
      message:
        "Update references environment-variable names; values are not inspected or displayed, and explicit approval is required.",
      paths: changed.filter(
        (path) =>
          collectEnvironmentNames(newFiles.get(path)?.toString("utf8") ?? "")
            .length > 0,
      ),
      names: [...envNames].sort(),
    });
  if (secretPaths.length)
    findings.push({
      ...
      category: "secret",
      message:
        "Content appears to contain embedded secret material; secret values are hidden.",
  ...
  if (instructionPaths.length)
    findings.push({
      ...
      message:
        "Content contains suspicious instruction patterns that require human review.",
