...
export async function createSnapshot(paths: string[]): Promise<Snapshot> {
  ...
  async function capture(path: string): Promise<void> {
    ...
    if (info.isFile()) {
      snapshot.files.push({ path, existed: true, content: (await readFile(path)).toString("base64"), encoding: "base64" });
      return;
    }
    if (!info.isDirectory()) throw new Error(`Refusing unsupported snapshot target: ${path}`);
    snapshot.files.push({ path, existed: true, directory: true });
    const entries = await readdir(path, { withFileTypes: true });
    for (const entry of entries) {
      const child = join(path, entry.name);
      if (entry.isSymbolicLink()) throw new Error(`Refusing to snapshot symlink: ${child}`);
      if (entry.isDirectory()) await capture(child);
      else if (entry.isFile()) snapshot.files.push({ path: child, existed: true, content: (await readFile(child)).toString("base64"), encoding: "base64" });
    }
  }
...
export async function restoreSnapshot(snapshot: Snapshot): Promise<void> {
  ...
  for (const directory of snapshot.files.filter((file) => file.existed && file.directory).sort((a, b) => a.path.length - b.path.length)) await mkdir(directory.path, { recursive: true });
...
export async function readSnapshot(id: string): Promise<Snapshot> {
  return JSON.parse(await readFile(join(loadoutHome(), "snapshots", `${id}.json`), "utf8")) as Snapshot;
}
