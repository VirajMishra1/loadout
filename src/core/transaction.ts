...
const journalPath = (directory: string): string => join(directory, "transaction.json");
...
function parseJournal(value: unknown, directory: string): TransactionJournal {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`Corrupt Loadout transaction journal in ${directory}`);
  const journal = value as Partial<TransactionJournal>;
  if (journal.version !== 1 || typeof journal.id !== "string" || !validId(journal.id) || typeof journal.snapshotId !== "string" || !validId(journal.snapshotId) || !Array.isArray(journal.targets) || !journal.targets.every((target) => typeof target === "string") || typeof journal.createdAt !== "string" || (journal.status !== "prepared" && journal.status !== "committing")) {
    throw new Error(`Corrupt Loadout transaction journal in ${directory}`);
  }
  if (journal.id !== directory.split(/[\\/]/).at(-1)) throw new Error(`Corrupt Loadout transaction journal in ${directory}`);
...
async function readJournal(directory: string): Promise<TransactionJournal> {
  try {
    ...
    if (error instanceof Error && error.message.startsWith("Corrupt Loadout transaction journal")) throw error;
    throw new Error(`Corrupt Loadout transaction journal in ${directory}`);
  }
}

export async function beginTransaction(snapshot: Snapshot, targets: string[]): Promise<TransactionHandle> {
  ...
  await writeJournal(directory, { version: 1, id: handle.id, snapshotId: handle.snapshotId, targets: [...new Set(targets)], createdAt: new Date().toISOString(), status: "prepared" });
  return handle;
}

export async function markTransactionCommitting(handle: TransactionHandle): Promise<void> {
  const journal = await readJournal(handle.directory);
  await writeJournal(handle.directory, { ...journal, status: "committing" });
}

export async function completeTransaction(handle: TransactionHandle): Promise<void> {
  await rm(handle.directory, { recursive: true, force: true });
}

export async function rollbackTransaction(handle: TransactionHandle): Promise<void> {
...
export async function recoverPendingTransactions(): Promise<string[]> {
  ...
  for (const entry of entries.sort()) {
    ...
    if (!info.isDirectory() || info.isSymbolicLink()) throw new Error(`Unsafe Loadout transaction staging entry: ${directory}`);
