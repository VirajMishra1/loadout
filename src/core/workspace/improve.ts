...
import { buildHealthReport } from "../reporting/health.js";
import { createHash } from "node:crypto";
import { mkdir, readFile, readdir, writeFile } from "node:fs/promises";
import { join, resolve } from "node:path";
import { loadoutHome } from "../agents/paths.js";
import { detectSecretKinds } from "../catalog/safety.js";
...
export function proposeImprovements(
  ...
  if (report.driftedFiles)
    proposals.push({
      ...
      outcome:
        "Every drifted file is attributed to a package and can be safely reconciled or adopted.",
      acceptanceTests: [
  ...
  if (report.driftedMcpServers)
    proposals.push({
      ...
      outcome:
        "Every MCP change is attributed and can be reviewed, restored, adopted, or removed without changing unrelated servers.",
      acceptanceTests: [
  ...
  if (report.updatesAvailable)
    proposals.push({
      ...
      outcome:
        "Produce reviewable update plans and safely apply approved updates.",
      acceptanceTests: [
  ...
  if (!report.installedPackages)
    proposals.push({
      ...
      outcome:
        "Offer an explained tested profile or project-aware recommendations.",
      acceptanceTests: [
  ...
  if (missingAgents.length)
    proposals.push({
      ...
      outcome:
        "Keep unsupported targets disabled and explain how to enable them.",
      acceptanceTests: [
...
export async function buildImprovementCycle(
  health: () => Promise<HealthReport> = buildHealthReport,
  ...
  for (const name of names) {
    try {
      const cycle = JSON.parse(
        await readFile(join(improvementHome(), name), "utf8"),
      ) as ImprovementCycle;
...
export async function writeImprovementCycle(
  cycle: ImprovementCycle,
  directory = improvementHome(),
): Promise<{ json: string; prompt: string }> {
...
export async function recordImprovementOutcome(
  ...
  if (note && detectSecretKinds(note).length)
    throw new Error(
      "Improvement outcome note appears to contain secret material",
    );
