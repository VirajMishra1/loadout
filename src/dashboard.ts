...
async function route(
  ...
  try {
    ...
    if (
      (pathname === "/api/sync-plan" || pathname === "/api/plan") &&
      request.method === "GET"
    ) {
    ...
    if (
      (pathname === "/api/sync" || pathname === "/api/apply") &&
      request.method === "POST"
    ) {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const port =
    process.env.LOADOUT_PORT === undefined
      ? 0
      : Number(process.env.LOADOUT_PORT);
  void startDashboardServer({}, port)
    .then((handle) =>
      console.log(`Loadout dashboard: http://127.0.0.1:${handle.port}`),
    )
    .catch((error: unknown) => {
