...
function agent(id: AgentId, displayName: string, home: string): DetectedAgent {
  return { id, displayName, installed: true, skillsDirectory: agentSkillsDirectory(id, home, "linux") };
}
...
describe("agent-managed component inspection", () => {
  ...
  it("inspects only declared local directories for all six adapters", async () => {
    ...
    const agents = definitions.map((item) => agent(item.id, item.displayName, root!));

    for (const item of agents) {
      const skill = agentComponentDirectory(item, "skill");
      expect(skill).toBeDefined();
      await mkdir(join(skill!, "example-skill"), { recursive: true });
      await writeFile(join(skill!, "example-skill", "SKILL.md"), "not executed");
    }
    for (const item of agents) {
    ...
    for (const entry of inventory) {
      const skill = entry.components.find((component) => component.type === "skill")!;
      expect(skill).toMatchObject({ compatibility: "native", scanned: true, directoryExists: true });
      expect(skill.entries).toContainEqual({ path: "example-skill/SKILL.md", kind: "file" });
    }

    const codex = inventory.find((entry) => entry.agent.id === "codex")!;
    expect(codex.components.find((component) => component.type === "command")).toMatchObject({ compatibility: "adapted", scanned: true, directory: join(root, ".codex", "prompts") });
    expect(codex.components.find((component) => component.type === "rule")).toMatchObject({ compatibility: "unsupported", scanned: false, entries: [] });
    expect(codex.components.find((component) => component.type === "mcp")).toMatchObject({ compatibility: "adapted", scanned: false, entries: [] });

    const hermes = inventory.find((entry) => entry.agent.id === "hermes")!;
    expect(hermes.components.find((component) => component.type === "command")).toMatchObject({ compatibility: "unsupported", scanned: false });
    expect(hermes.components.find((component) => component.type === "plugin")?.note).toContain("runtime behavior");
  });
  ...
  it("records but never follows symlinks in a managed root", async () => {
    ...
    const skills = inspected[0].components.find((component) => component.type === "skill")!;
    expect(skills.entries).toContainEqual({ path: "outside", kind: "symlink" });
    expect(skills.entries).not.toContainEqual({ path: "outside/anything", kind: "file" });
