...
function detectedAgent(id: AgentId, displayName: string, home: string): DetectedAgent {
...
describe("native per-agent skill planners", () => {
  ...
  it.each(ADAPTERS)("plans, installs, and removes a native skill for $displayName", async ({ id, displayName }) => {
    ...
    const plan = await planAdapterSkillInstall(source, "adapter-smoke", agent);
    ...
    expect(await readFile(join(agent.skillsDirectory, "adapter-smoke", "SKILL.md"), "utf8")).toContain("adapter-smoke");

    const removal = await planRemove("adapter-smoke");
    expect(removal.blocked).toBe(false);
    await applyRemove(removal);
    await expect(readFile(join(agent.skillsDirectory, "adapter-smoke", "SKILL.md"))).rejects.toThrow();
  });
});
