...
import { startApiServer, type ApiHandle } from "../src/core/runtime/api.js";
...
describe("local API", () => {
  it("serves read-only endpoints on an ephemeral loopback port", async () => {
    ...
    expect((await get("/status")).body).toEqual({
      ok: true,
      data: [{ id: "codex", installed: true }],
    });
  ...
  it("returns structured errors for unknown paths, methods, and handler failures", async () => {
    ...
    expect((await get("/missing")).body).toEqual({
      ok: false,
      error: { code: "NOT_FOUND", message: "Endpoint not found." },
    });
    expect((await get("/status")).body).toEqual({
      ok: false,
      error: { code: "INTERNAL_ERROR", message: "state unavailable" },
    });
    const response = await fetch(
      `http://${handle.host}:${handle.port}/status`,
      { method: "POST" },
    );
  ...
  it("refuses non-loopback bindings", async () => {
    await expect(startApiServer({ host: "0.0.0.0", port: 0 })).rejects.toThrow(
      /loopback-only/,
    );
  });
});
