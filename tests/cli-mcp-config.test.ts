...
describe("mcp-config CLI", () => {
  it("shows help and keeps dry-run output free of environment values", async () => {
    const help = await run(cli, [entry, "mcp-config", "--help"]);
    ...
    try {
      const result = await run(cli, [entry, "mcp-config", "--config", config, "--name", "docs", "--command", "npx", "--arg", "-y", "--env", "TOKEN=secret-value"]);
