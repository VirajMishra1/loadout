...
import { applyCodexMcpConfigPlan, planCodexMcpConfig } from "../src/core/codex-mcp.js";
import type { McpServer } from "../src/shared/types.js";

let root: string;
beforeEach(async () => { root = await mkdtemp(join(tmpdir(), "loadout-codex-mcp-")); process.env.LOADOUT_HOME = join(root, ".loadout"); });
afterEach(async () => { delete process.env.LOADOUT_HOME; await rm(root, { recursive: true, force: true }); });

const server: McpServer = { name: "context7", command: "npx", args: ["-y", "@upstash/context7-mcp"], env: { API_TOKEN: "secret" }, sourcePath: "mcp.json", warnings: [] };
...
describe("Codex TOML MCP configuration", () => {
  ...
  it("refuses to overwrite an existing server table", async () => {
    ...
    await expect(planCodexMcpConfig(path, server)).rejects.toThrow("already exists");
  });
});
