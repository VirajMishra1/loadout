...
describe("universal package components", () => {
  ...
  it("discovers conventional rules, commands, and agents", async () => {
    ...
    for (const directory of ["rules", "commands", "agents"])
    ...
    const agents: DetectedAgent[] = [
      {
        ...
        installed: true,
        skillsDirectory: join(home, ".claude", "skills"),
      },
      {
        id: "codex",
        displayName: "Codex",
        installed: true,
        skillsDirectory: join(home, ".agents", "skills"),
      },
      {
        id: "cursor",
        displayName: "Cursor",
        installed: true,
        skillsDirectory: join(home, ".cursor", "skills"),
      },
    ];
    const plan = await buildUniversalPackagePlan(root, "demo", agents);
    expect(
      plan.files.filter((file) => file.componentType === "command"),
    ).toHaveLength(3);
    expect(plan.files).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          target: join(home, ".codex", "prompts", "demo", "review.md"),
          compatibility: "adapted",
        }),
      ]),
    );
    expect(
      plan.files.filter((file) => file.componentType === "rule"),
    ).toHaveLength(1);
  ...
  it("allows explicit scoped root exports and rejects escaping targets", async () => {
    ...
    await addRootFileExports(plan, root, join(root, "project"), [
      { source: "AGENTS.md", target: "AGENTS.md" },
    ]);
    expect(plan.files).toEqual([
      expect.objectContaining({
        source: join(root, "AGENTS.md"),
        target: join(root, "project", "AGENTS.md"),
        componentType: "root",
      }),
    ]);
    await expect(
      addRootFileExports(plan, root, join(root, "project"), [
        { source: "AGENTS.md", target: "../escape.md" },
      ]),
    ).rejects.toThrow(/escapes allowed scope/);
  });
});
