...
describe("isolated demo mode", () => {
  ...
  it("installs and rolls back inside a disposable profile without changing process configuration", async () => {
    ...
    const result = await runIsolatedDemo({ source, packageId: "demo-skill", agents: ["codex"] });
    ...
    expect(process.env.LOADOUT_USER_HOME).toBe("/real-user-home-must-not-be-touched");
    expect(process.env.LOADOUT_HOME).toBe("/real-loadout-home-must-not-be-touched");
  });

  it("can retain a still-isolated profile for a presenter to inspect", async () => {
    const source = await createSkillSource();
    const result = await runIsolatedDemo({ source, packageId: "demo-skill", agents: ["codex", "claude-code"], keep: true });
    directories.push(result.profile);

    expect(result.rolledBack).toBe(false);
    expect(result.cleanedUp).toBe(false);
    expect(await readFile(join(result.profile, ".agents", "skills", "demo-skill", "SKILL.md"), "utf8")).toContain("demo-skill");
    expect(await readFile(join(result.profile, ".claude", "skills", "demo-skill", "SKILL.md"), "utf8")).toContain("demo-skill");
  });
});
