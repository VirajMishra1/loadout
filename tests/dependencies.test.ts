...
describe("registry package dependencies", () => {
  ...
  async function publish(
    ...
    const descriptor = await createPackage(directory, {
      name,
      version,
    ...
    await writeFile(
      ...
      `${JSON.stringify({ ...descriptor, ...options }, null, 2)}\n`,
    );
  ...
  it("installs and locks transitive production plus opted-in development dependencies", async () => {
    root = await mkdtemp(join(tmpdir(), "loadout-dependencies-"));
    process.env.LOADOUT_HOME = join(root, ".loadout");
    await publish("base", "1.0.0");
    await publish("devtool", "1.0.0");
    ...
    const path = await manifest([
      {
        id: "app",
        source: { type: "registry", name: "app", version: "1.0.0" },
        includeDevDependencies: true,
      },
    ]);
    const plan = await buildSyncPlan(path);
    expect(plan.packages.map((entry) => entry.plan.packageId)).toEqual([
      "devtool",
      "base",
      "middle",
      "app",
    ...
    await applySyncPlan(plan, lockPath);
    const lock = JSON.parse(await readFile(lockPath, "utf8"));
    expect(lock.packages.map((pkg: { id: string }) => pkg.id).sort()).toEqual([
      "app",
      "base",
      "devtool",
      "middle",
    ...
    expect(
      lock.packages
        .find((pkg: { id: string }) => pkg.id === "app")
        .dependencies.sort(),
    ).toEqual(["devtool", "middle"]);
    expect(
      lock.packages.find((pkg: { id: string }) => pkg.id === "middle")
        .dependencies,
    ).toEqual(["base"]);
  });

  it("does not install development dependencies unless requested", async () => {
    root = await mkdtemp(join(tmpdir(), "loadout-dev-dependencies-"));
    process.env.LOADOUT_HOME = join(root, ".loadout");
    await publish("devtool", "1.0.0");
    await publish("app", "1.0.0", { devDependencies: { devtool: "1.0.0" } });
  ...
  it("rejects dependency cycles and incompatible transitive versions", async () => {
    root = await mkdtemp(join(tmpdir(), "loadout-dependency-errors-"));
    process.env.LOADOUT_HOME = join(root, ".loadout");
    await publish("a", "1.0.0", { dependencies: { b: "1.0.0" } });
    await publish("b", "1.0.0", { dependencies: { a: "1.0.0" } });
    await expect(
      buildSyncPlan(
        await manifest([
          {
            id: "a",
            source: { type: "registry", name: "a", version: "1.0.0" },
          },
        ]),
      ),
    ).rejects.toThrow(/dependency cycle/);
    await publish("shared", "1.0.0");
    await publish("shared", "2.0.0");
    await publish("left", "1.0.0", { dependencies: { shared: "1.0.0" } });
    await publish("right", "1.0.0", { dependencies: { shared: "2.0.0" } });
    await publish("conflict", "1.0.0", {
    ...
    await expect(
      buildSyncPlan(
        await manifest([
          {
            id: "conflict",
            source: { type: "registry", name: "conflict", version: "1.0.0" },
          },
        ]),
      ),
