...
describe("manifest and lockfile", () => {
  ...
  it("creates and validates a shareable manifest", async () => {
    const root = await mkdtemp(join(tmpdir(), "loadout-manifest-"));
    roots.push(root);
    const path = join(root, "loadout.json");
    await initManifest(path, { name: "team", agents: ["codex"] });
    expect(await readManifest(path)).toMatchObject({
      schemaVersion: 1,
      name: "team",
      agents: ["codex"],
      packages: [],
  ...
  it("writes a lockfile from managed state without secrets", async () => {
    const root = await mkdtemp(join(tmpdir(), "loadout-lock-"));
    roots.push(root);
    ...
    const manifest = parseManifest({
      ...
      agents: ["codex"],
      packages: [
        { id: "demo", source: { type: "github", repository: "owner/repo" } },
      ],
    });
  ...
  it("adds, removes, and applies profiles without duplicating packages", async () => {
    const root = await mkdtemp(join(tmpdir(), "loadout-manifest-edit-"));
    roots.push(root);
    const path = join(root, "loadout.json");
    await initManifest(path, { name: "team", agents: ["codex"] });
    await addManifestPackage(path, {
      id: "one",
      source: { type: "github", repository: "owner/one", ref: "v1.0.0" },
    });
    await expect(
      addManifestPackage(path, {
        id: "one",
        source: { type: "catalog", id: "one" },
      }),
    ).rejects.toThrow(/already contains/);
    await applyProfileToManifest(path, "stable", [
      { id: "one", repository: "owner/one" },
      { id: "two", repository: "owner/two" },
    ]);
    expect((await readManifest(path)).packages.map((pkg) => pkg.id)).toEqual([
      "one",
      "two",
    ]);
  ...
  it("orders dependencies and rejects missing or cyclic dependency graphs", () => {
    const dependency = {
      id: "base",
      source: { type: "catalog" as const, id: "base" },
    };
    const app = {
      id: "app",
    ...
    expect(
      orderManifestPackages([app, dependency]).map((pkg) => pkg.id),
    ).toEqual(["base", "app"]);
    expect(() =>
      parseManifest({
        schemaVersion: 1,
        ...
        scope: "project",
        agents: ["codex"],
        packages: [{ ...app, dependsOn: ["missing"] }],
      }),
  ...
  it("validates explicit MCP targets without guessing agent config paths", () => {
    const manifest = parseManifest({
      ...
      packages: [
        {
          id: "docs",
          source: { type: "catalog", id: "context7" },
          mcp: { config: ".config/mcp.json", servers: ["docs"] },
        },
      ],
    });
    expect(manifest.packages[0].mcp).toEqual({
    ...
    expect(() =>
      parseManifest({
        ...
        agents: ["codex"],
        packages: [
          { id: "docs", source: { type: "catalog", id: "context7" }, mcp: {} },
        ],
      }),
    ).toThrow(/mcp.config/);
  });
      ...
      packages: [
        {
          ...
          rootFiles: [{ source: "AGENTS.md", target: "AGENTS.md" }],
        },
      ],
    });
    expect(manifest.packages[0].rootFiles).toEqual([
