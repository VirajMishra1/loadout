...
import { applyMcpConfigPlan, planMcpConfig, restoreMcpConfig, summarizeMcpConfigPlan } from "../src/core/mcp.js";
import type { McpServer } from "../src/shared/types.js";

let root: string;
beforeEach(async () => { root = await mkdtemp(join(tmpdir(), "loadout-mcp-config-")); process.env.LOADOUT_HOME = join(root, ".loadout"); });
afterEach(async () => { delete process.env.LOADOUT_HOME; await rm(root, { recursive: true, force: true }); });

const server: McpServer = { name: "docs", command: "npx", args: ["-y", "server"], env: { TOKEN: "super-secret" }, sourcePath: "repo/mcp.json", warnings: [] };

it("plans while preserving unrelated keys and redacting secrets in output", async () => {
  const path = join(root, "claude.json");
  await writeFile(path, JSON.stringify({ theme: "dark", mcpServers: { old: { command: "old" } } }));
...
it("rejects malformed server names", async () => {
  await expect(planMcpConfig(join(root, "x.json"), server, "bad/name")).rejects.toThrow();
});
