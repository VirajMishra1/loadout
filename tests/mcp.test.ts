...
describe("MCP manifest discovery", () => {
  it("normalizes mcpServers and never exposes env values in summaries", () => {
    const manifest = parseMcpManifest({ mcpServers: { docs: { command: "npx", args: ["-y", "server"], env: { TOKEN: "secret" } }, remote: { url: "https://example.test/mcp" } } }, "/tmp/mcp.json");
    expect(manifest.servers[0]).toMatchObject({ name: "docs", command: "npx", args: ["-y", "server"], env: { TOKEN: "secret" } });
  ...
  it("finds supported manifests recursively and reports malformed JSON", async () => {
    ...
    expect(manifests.find((entry) => entry.servers.length === 1)?.servers[0].name).toBe("local");
    expect(manifests.find((entry) => entry.warnings.length > 0)?.warnings[0]).toContain("invalid JSON");
  });
});
