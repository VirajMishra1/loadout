...
describe("package inspection", () => {
  it("returns normalized skills and MCP servers without secret values", async () => {
    ...
    expect(
      result.plugins.find((plugin) => plugin.name === "docs-plugin"),
    ).toMatchObject({
      type: "plugin",
      name: "docs-plugin",
      version: "1.2.0",
      components: ["command", "mcp", "skill"],
      hookEvents: ["SessionStart"],
      mcpServers: ["context"],
    });
    expect(
      result.plugins.find((plugin) => plugin.name === "codex-plugin"),
    ).toMatchObject({ components: ["agent"] });
