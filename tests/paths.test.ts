...
describe("platform paths", () => {
  ...
  it("uses APPDATA for Windows state and has a profile fallback", () => {
    expect(
      loadoutHome(
        {
          USERPROFILE: "C:\\Users\\viraj",
          APPDATA: "C:\\Users\\viraj\\AppData\\Roaming",
        },
        "win32",
      ),
    ).toBe(win32.join("C:\\Users\\viraj\\AppData\\Roaming", "loadout"));
    expect(loadoutHome({ USERPROFILE: "C:\\Users\\viraj" }, "win32")).toBe(
      win32.join("C:\\Users\\viraj", "AppData", "Roaming", "loadout"),
    );
  });
  ...
  it("uses native separators for every adapter filesystem layout", () => {
    ...
    for (const definition of AGENT_DEFINITIONS) {
      expect(agentSkillsDirectory(definition.id, windowsHome, "win32")).toBe(
        win32.join(windowsHome, ...definition.directory),
      );
      expect(agentSkillsDirectory(definition.id, posixHome, "linux")).toBe(
        posix.join(posixHome, ...definition.directory),
      );
    }
  });
  ...
  it("looks up npm .cmd shims on Windows without changing POSIX lookup", () => {
    ...
    expect(executableLookup("codex", "win32")).toEqual({
      command: "where",
      candidates: ["codex", "codex.cmd", "codex.exe", "codex.bat"],
    });
    expect(executableLookup("codex", "linux")).toEqual({
      command: "which",
      candidates: ["codex"],
    });
  });
  ...
  it("honors explicit state-directory overrides on every platform", () => {
    expect(
      loadoutHome(
        { LOADOUT_HOME: "/tmp/loadout-state", APPDATA: "ignored" },
        "win32",
      ),
    ).toBe("/tmp/loadout-state");
  });
