...
const base: CatalogPackage = {
  id: "evidenced-package",
  displayName: "Evidenced Package",
  repository: "example/evidenced-package",
  description: "test",
  category: "test",
  tier: "stable",
  stars: 1_000_000,
  ...
  source: {
    type: "github",
    url: "https://github.com/example/evidenced-package",
    defaultBranch: "main",
    commit: "a".repeat(40),
...
describe("catalog score explanation", () => {
  it("caps and logarithmically scales popularity instead of allowing stars to dominate", () => {
    ...
    expect(
      score.contributions.find((item) => item.factor === "adoption"),
    ).toMatchObject({ maximum: 30, points: 30 });
    expect(
      score.contributions.find((item) => item.factor === "momentum"),
    ).toMatchObject({
      points: 0,
      evidence: expect.stringMatching(/single snapshot/),
    });
  ...
  it("does not mistake catalog verification or metadata updates for code maintenance", () => {
    const score = explainCatalogScore(
      {
        ...base,
        stars: undefined,
        pushedAt: undefined,
    ...
    expect(
      score.contributions.find((item) => item.factor === "maintenance"),
    ).toMatchObject({
      points: 0,
      evidence: expect.stringMatching(/code-push timestamp/),
    });
    expect(
