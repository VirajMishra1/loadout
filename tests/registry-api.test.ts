...
describe("remote registry protocol", () => {
  ...
  it("authenticates immutable publishing and verifies exact-version downloads", async () => {
    ...
    await writeFile(
      join(packageRoot, "skills", "demo.md"),
      "Verified remote package.\n",
    );
    handle = await startRegistryServer({ token: "test-secret" });
    const registry = `http://${handle.host}:${handle.port}`;
    await expect(
      publishRemotePackage(packageRoot, registry, "wrong-secret"),
    ).rejects.toThrow(/Invalid registry token/);
    ...
    expect(
      await readFile(join(fetched.path, "skills", "demo.md"), "utf8"),
    ).toContain("Verified");
    await writeFile(
      join(packageRoot, "skills", "demo.md"),
      "Changed content.\n",
    ...
    await expect(
      publishRemotePackage(packageRoot, registry, "test-secret"),
    ).rejects.toThrow(/already exists with different content/);
  });

  it("requires HTTPS for non-loopback clients", async () => {
    await expect(
      fetchRemoteRegistryPackage("http://example.com", "demo", "1.0.0"),
  ...
  it("keeps risk approval explicit for remote publishing", async () => {
    ...
    await writeFile(
      join(packageRoot, "commands", "install.sh"),
