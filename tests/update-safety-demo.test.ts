...
function codexAgent(skillsDirectory: string): DetectedAgent {
  return { id: "codex", displayName: "Codex", binary: "codex", installed: true, skillsDirectory };
}
...
describe("safe update demo fixtures", () => {
  ...
  async function setup(candidate: "benign-v2" | "risky-v2") {
    ...
    const cache = join(process.env.LOADOUT_HOME, "cache", "owner__ponytail", oldCommit);
    ...
    await cp(join(fixtureRoot, "benign-v1"), join(target, "ponytail-demo"), { recursive: true });
    ...
    return {
      ...
      source,
      runtime: {
        fetchSnapshot: async () => ({ repository: "owner/ponytail", commit: newCommit, path: source }),
  ...
  it("accepts a benign Ponytail-style documentation update without executing repository code", async () => {
    ...
    expect(await readFile(join(demo.target, "ponytail-demo", "SKILL.md"), "utf8")).toContain("Summarize the affected files");
    expect((await readInstallState()).installs[0].resolvedCommit).toBe(newCommit);
    expect(await exists(join(process.env.LOADOUT_HOME!, "fixture-hook-was-executed"))).toBe(false);
  });
  ...
  it("quarantines a risky hook and domain update until the user supplies explicit approval", async () => {
    ...
    await expect(applyPackageUpdate("ponytail-demo", {}, demo.runtime)).rejects.toThrow(/blocked pending explicit risk approval.*quarantined at/);

    const metadata = JSON.parse(await readFile(join(process.env.LOADOUT_HOME!, "quarantine", `ponytail-demo-${newCommit}`, "metadata.json"), "utf8"));
    expect(metadata.findings.map((finding: { category: string }) => finding.category)).toEqual(expect.arrayContaining(["hook", "script", "domain"]));
    expect(await readFile(join(demo.target, "ponytail-demo", "SKILL.md"), "utf8")).not.toContain("reviewEndpoint");
    expect((await readInstallState()).installs[0].resolvedCommit).toBe(oldCommit);
    expect(await exists(join(process.env.LOADOUT_HOME!, "fixture-hook-was-executed"))).toBe(false);
  });

  it("restores the prior files and install state after a simulated static smoke-test failure", async () => {
    const demo = await setup("benign-v2");
    await expect(applyPackageUpdate("ponytail-demo", {}, {
      ...demo.runtime,
      verify: async () => { throw new Error("simulated smoke-test failure"); }
    })).rejects.toThrow(/verification failed; restored snapshot/);

    const restored = await readFile(join(demo.target, "ponytail-demo", "SKILL.md"), "utf8");
    expect(restored).toBe(await readFile(join(fixtureRoot, "benign-v1", "SKILL.md"), "utf8"));
    expect((await readInstallState()).installs[0].resolvedCommit).toBe(oldCommit);
  });
});
